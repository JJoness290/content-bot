...
from app.core.video_pipeline import ScriptItem, generate_video_for_script
...
def _build_script_payload(idea: dict[str, Any], platform: str) -> dict[str, str]:
    title = idea["topic"] if platform == "youtube" else ""
    script_body = build_script(idea, platform)
    return {
        "hook": idea["hook"],
        "body": script_body,
...
def _enqueue_video_script(platform: str) -> dict[str, Any] | None:
    if platform not in {"tiktok", "youtube"}:
        return None
    try:
        init_video_queue_db()
        idea = decide_next_content(platform)
        logger.info("Autopilot idea (%s): %s", platform, idea)
        payload = _build_script_payload(idea, platform)
        logger.info("Autopilot script (%s): %s", platform, payload["body"])
        return create_script_item(platform, idea["topic"], payload)
    except Exception as exc:
        logger.warning("Failed to enqueue video script for %s: %s", platform, exc)
        return None


def _has_video_output(script_id: int, platform: str) -> bool:
    outputs = list_outputs_for_script(script_id)
    expected_kind = "VIDEO_MP4" if platform == "tiktok" else "SHORT_VIDEO_MP4"
    return any(output.get("kind") == expected_kind for output in outputs)


def _autonomous_video_tick() -> list[str]:
    results: list[str] = []
    for platform in ("tiktok", "youtube"):
        scripts = list_scripts(platform)
        if not scripts:
            created = _enqueue_video_script(platform)
            if created:
                scripts = [created]
                results.append(f"Queued {platform} script.")
        if not scripts:
            continue
        pending = next((item for item in scripts if not _has_video_output(item["id"], platform)), None)
        if not pending:
            results.append(f"No pending scripts for {platform}.")
            continue
        try:
            payload = json.loads(pending["payload_json"] or "{}")
            generate_video_for_script(
                ScriptItem(id=pending["id"], platform=platform, payload=payload)
            )
            results.append(f"Generated {platform} video for script #{pending['id']}.")
        except Exception as exc:
            logger.warning("Failed generating %s video: %s", platform, exc)
    return results
...
async def autopilot_loop(stop_event: asyncio.Event, interval_seconds: int = AUTOPILOT_INTERVAL_SECONDS) -> None:
    ...
    while not stop_event.is_set():
        ...
        if state["enabled"]:
            ...
            for message in enqueue_result:
                logger.info("Content autopilot video: %s", message)
        else:
